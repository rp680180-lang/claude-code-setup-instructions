...
The full plugin JSON contains complete manifests and is too large to process reliably. Use shell commands to keep the data small and let the shell do the matching — not your in-context reasoning over a large blob.

**Step 1 — Add the marketplace and get a compact catalog (names + descriptions only):**

```bash
claude plugin marketplace add anthropics/claude-plugins-official 2>/dev/null
claude plugin list --available --json | jq '[.[] | {name: .name, description: (.description // "")}]'
```

Confirm this returns results before continuing. If it returns empty, retry once. If it still fails, fall back to recommending only the core workflow plugins by name (listed in Phase 2).

**Step 2 — Search for each detected service/tool by keyword:**

For each significant signal found in Phase 1b (services, languages, frameworks, tools), run a targeted search:

```bash
claude plugin list --available --json | jq '[.[] | select((.name + " " + (.description // "")) | test("KEYWORD"; "i")) | {name, description}]'
```

Replace `KEYWORD` with each detected signal — for example:
- Firebase in deps → keyword `firebase`
- `.github/` exists → keyword `github`
- `.gitlab-ci.yml` → keyword `gitlab`
- TypeScript detected → keyword `typescript`
- Stripe deps → keyword `stripe`
- Playwright detected → keyword `playwright`

Run a separate search for each distinct signal. Collect all matching plugins as recommendation candidates.

This approach is reliable because the shell does the matching — you only decide which keywords to search for based on what was detected.

---

## Phase 2: Plan (single approval checkpoint)

Using the project profile from Phase 1 and the plugin matches from Phase 1d, build one unified plan. **Every recommendation must be derived from what was actually detected**, not from a fixed checklist.
...
2. *Stack-matched*: All plugins returned by the keyword searches in Phase 1d. Only recommend from `anthropics/claude-plugins-official`.
